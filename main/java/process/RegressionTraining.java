...
public class RegressionTraining {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length > 0) {
            ...
            window = Integer.parseInt(args[8]);
            updaterString = args[9];

            boolean match = Boolean.parseBoolean(args[10]);
            ...
            if (!test) {
                ...
                if (match) {
                    ...
                    configuration = getRegrissionNetworkConfiguration(1);
                } else {
                ...
                File locationToSave;
                if (match) {
                    locationToSave = new File(workingDir + File.separator + "matchModel_" + minTokenLimit + "_" + maxTokenLimit + "_" + tokensLimitCount + "_" + learningRate + "_" + nEpochs + ".zip");       //Where to save the network. Note: the file is in .zip format - can be opened externally
                } else {
                    locationToSave = new File(workingDir + File.separator + "facetModel_" + minTokenLimit + "_" + maxTokenLimit + "_" + tokensLimitCount + "_" + learningRate + "_" + nEpochs + ".zip");       //Where to save the network. Note: the file is in .zip format - can be opened externally
                }
                ...
                matchModel = new ComputationGraph(getRegrissionNetworkConfiguration(1));
                ...
                File locationToSaveMatch = new File(workingDir + File.separator + "matchModel_" + minTokenLimit + "_" + maxTokenLimit + "_" + tokensLimitCount + "_" + learningRate + "_" + nEpochs + ".zip");       //Where to save the network. Note: the file is in .zip format - can be opened externally
                File locationToSaveFacet = new File(workingDir + File.separator + "facetModel_" + minTokenLimit + "_" + maxTokenLimit + "_" + tokensLimitCount + "_" + learningRate + "_" + nEpochs + ".zip");       //Where to save the network. Note: the file is in .zip format - can be opened externally

                boolean saveUpdater = true;                                             //Updater: i.e., the state for Momentum, RMSProp, Adagrad etc. Save this if you want to train your network more in the future
                ModelSerializer.writeModel(matchModel, locationToSaveMatch, saveUpdater);
                ModelSerializer.writeModel(facetModel, locationToSaveFacet, saveUpdater);
    ...
    private static ComputationGraphConfiguration getRegrissionNetworkConfiguration(int outputSize) {
